/**
 * Created by icebr on 2016/5/19.
 */
$(document).ready(function(){
    pintuerInit();
});
function indexAjaxAsk(th_a, option) {
    var opts = $.extend({
        _mask: '', /* 临时变量，不需要赋值 */
    ...
    $.ajax({
                ...
                if (opts.layer.btn) {
                    ...
                    var _layerOpts = $.extend({
                        ...
                        yes: function (index, layero) {
                            layer.closeAll();
                            indexAjaxAsk(opts._url, $.extend(opts, {ask: 99}));
                        },
        ...
        success: function (data) {
            ...
            if (99 == opts.ask) {
                ...
                if (data.indexOf('admin-login-login-admin') != -1) {
                ...
                _layerOpts.ask == 98 ?
                    layer.msg(_layerOpts.content, _layerOpts, function () {
                        if (_layerOpts.reload) {
                            indexAjaxAsk(opts._url, {reload: true});
                        } else if (_layerOpts.refresh) {
                            if(_layerOpts.refresh == 1){
                                window.location.reload();
                            }else{
                                window.location.href = _layerOpts.refresh;
                            }
                        } else {
                            indexAjaxAsk(opts._url, {history: true});
                        }
                    }) :
...
function indexAjaxCookie(th_a, opts) {
...
$(document).ready(function() {
    ...
    /*字数限制*/
    $(".checkWordNum").on("propertychange input", function() {
        var $this = $(this),
            _val = $this.val(),
            count = "";
        if (_val.length > 140) {
            $this.val(_val.substring(0, 140));
        }
        count = 140 - $this.val().length;
        $("#text-count").text(count);
    });

    $(".checkLnWord").on("propertychange input", function() {
        var $this = $(this),
            _val = $this.val(),
            count = "";
        if (_val.length > 700) {
            $this.val(_val.substring(0, 700));
        }
        count = 700 - $this.val().length;
        $("#text-count").text(count);
    });

    $(".scroll_left").find(".layui-nav-item").click(function() {
        $(".scroll_left").find(".layui-nav-item").removeClass('layui-nav-itemed')
        $(this).addClass('layui-nav-itemed'); 
    })
    $(".scroll_left").find("a").focus(function() {
        $(this).blur();
    });
    $(".menu_ul").find("a").focus(function() {
        $(this).blur();
    });
    
});
//by sam de js end
function pintuerInit(){
    // 选项卡
